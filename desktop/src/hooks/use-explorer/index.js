...
export function useExplorer(options = {}) {
  ...
  watch(
    ...
    async () => {
      if (deviceId.value) {
        ...
        // Record directory visit for quick access
        const currentPathValue = pathManager.currentPath.value
        if (currentPathValue) {
          quickAccess.touch({
            id: currentPathValue,
            name: pathManager.getBasename(currentPathValue),
            type: 'directory',
  ...
  return {
    ...
    sortOptions: fileOps.sortOptions,
    refresh,
    ...
    quickAccess: {
      ...
      recentItems: quickAccess.recentItems,
      favoriteIds: quickAccess.favoriteIds,
      addFavorite: quickAccess.addFavorite,
      removeFavorite: quickAccess.removeFavorite,
      toggleFavorite: quickAccess.toggleFavorite,
      isFavorite: quickAccess.isFavorite,
      touch: quickAccess.touch,
      pendingHighlightId: quickAccess.pendingHighlightId,
      clearPendingHighlight: quickAccess.clearPendingHighlight,
      open: item => quickAccess.open(item, {
        validate: async item => fileOps.exists(item.id),
        onOpen: async item => navigateTo(item.id),
      }),
      clearRecent: quickAccess.clearRecent,
      load: quickAccess.load,
      save: quickAccess.save,
